...
namespace SLZ.CustomStaticBatching
{
		...
		public unsafe struct TransferVtxBuffer : IJobParallelFor
		{
			...
			[ReadOnly]
			public int4 offset_strideIn_TanSign;
			[ReadOnly]
			...
			{
				uint id = (uint)i;
				uint vertInAdr = (id * asuint(offset_strideIn_TanSign.y));
				uint vertOutAdr = (id * asuint(strideOut.x) + asuint(offset_strideIn_TanSign.x));

				WritePositionChannel(ref vertIn, ref vertOut, inPackedChannelInfo[0].packedData, outPackedChannelInfo[0].packedData, vertInAdr, vertOutAdr);
				WriteNormalChannel(ref vertIn, ref vertOut, inPackedChannelInfo[1].packedData, outPackedChannelInfo[1].packedData, vertInAdr, vertOutAdr);
				WriteTangentChannel(ref vertIn, ref vertOut, inPackedChannelInfo[2].packedData, outPackedChannelInfo[2].packedData, vertInAdr, vertOutAdr);
				WriteColorChannel(ref vertIn, ref vertOut, inPackedChannelInfo[3].packedData, outPackedChannelInfo[3].packedData, vertInAdr, vertOutAdr);

				for (int r = 4; r < 12; r++)
				{
					WriteUVChannel(ref vertIn, ref vertOut, inPackedChannelInfo[r].packedData, outPackedChannelInfo[r].packedData, vertInAdr, vertOutAdr, (r == 5), (r == 6));
				}
			}
			...
			void WritePositionChannel(ref NativeArray<byte> vertIn, ref NativeArray<byte> vertOut, uint inPackedInfo, uint outPackedInfo, uint vertInAdr, uint vertOutAdr)
			{
				...
				WriteValueVtx(ref vertOut, outAddress, FORMAT_FLOAT, 3, rawData);
			}

			void WriteNormalChannel(ref NativeArray<byte> vertIn, ref NativeArray<byte> vertOut, uint inPackedInfo, uint outPackedInfo, uint vertInAdr, uint vertOutAdr)
			{
				...
				if (GetDimension(inPackedInfo) > 0u) // output mesh guaranteed to have the channel if an input mesh has it
				{
					...
					WriteValueNorm(ref vertOut, outAddress, outFmt, outDimension, rawData);
				}
			}

			void WriteTangentChannel(ref NativeArray<byte> vertIn, ref NativeArray<byte> vertOut, uint inPackedInfo, uint outPackedInfo, uint vertInAdr, uint vertOutAdr)
			{
				if (GetDimension(inPackedInfo) > 0u) // output mesh guaranteed to have the channel if an input mesh has it
				{
					uint address = vertInAdr + GetOffset(inPackedInfo);
					...
					WriteValueTanColor(ref vertOut, outAddress, outFmt, 4, rawData);
				}
			}

			void WriteColorChannel(ref NativeArray<byte> vertIn, ref NativeArray<byte> vertOut, uint inPackedInfo, uint outPackedInfo, uint vertInAdr, uint vertOutAdr)
			{
				if ((outPackedInfo & 0x0000000fu) > 0u) // input mesh not guaranteed to have color even if the output does, thus we have to initialize it to 1,1,1,1 if it doesn't 
				{
					...
					WriteValueTanColor(ref vertOut, outAddress, outFmt, outDimension, rawData);
				}
			}

			void WriteUVChannel(ref NativeArray<byte> vertIn, ref NativeArray<byte> vertOut, uint inPackedInfo, uint outPackedInfo, uint vertInAdr, uint vertOutAdr, bool lmScaleOffset, bool dynLmScaleOffset)
			{
				if (GetDimension(inPackedInfo) > 0u) // output mesh guaranteed to have the channel if an input mesh has it
				{
					uint address = vertInAdr + GetOffset(inPackedInfo);
					...
					uint inDimension = GetDimension(inPackedInfo);
					//uint inByteEnum = GetByteCountEnum(inPackedInfo);
					WriteValue(ref vertOut, outAddress, outFmt, inDimension, rawData);
				}
			}
		}
}